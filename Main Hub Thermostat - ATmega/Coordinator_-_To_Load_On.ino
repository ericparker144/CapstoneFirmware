...
{    
   ...
   //Normal Operation
    if ( digitalRead(8)== LOW) { //Digital pin 8 is low then normal operation 
    ...
    // Pairing Process
    if ((digitalRead(8)== HIGH) && (num_active_nodes < max_active_nodes)) { //D8 is high and we don't have more than 5 routers connected
  ...
  if(payload2.Node01address == 0) {
    ...
    //  Serial.print(" Battery status: ");
     ...
     // Serial.print(" Battery status: ");
      ...
      //if(Serial) {
        Serial.print("{address:");
        Serial.print(payload2.Node01address,OCT); //This is the logical 011 address of the 01 end devices;
        Serial.print(",batState:");
        Serial.print(payload2.batState);
        Serial.print(",temp:");
        Serial.print(0); // 0 for end devices
