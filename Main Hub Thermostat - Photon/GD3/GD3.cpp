...
//#if !defined(ARDUINO) && !defined(__DUE__)
    ...
    if ((EEPROM.read(10) != 0x7d)) {
      ...
      for (int i = 0; i < 24; i++)
        GDTR.wr(REG_TOUCH_TRANSFORM_A + i, EEPROM.read(11 + i));
//    }
//#endif
  ...
  Serial1.println("GD.begin complete");
}
...
// Serial1.println("FT812");
 ...
 cmd_text(GD.w / 2, (GD.h / 2)+(2*(GD.h / 20)), 27, OPT_CENTERX, "CLCS Screen");
  ...
  cmd_text(GD.w / 2, GD.h / 2, 27, OPT_CENTERX, "Press on the dot to calibrate the touch screen");
...
{
  {
    ...
    Serial1.write(0xa5);
    Serial1.write(GD.w & 0xff);
    Serial1.write((GD.w >> 8) & 0xff);
    Serial1.write(GD.h & 0xff);
    Serial1.write((GD.h >> 8) & 0xff);
      ...
      for (int x = 0; x < GD.w; x += 8) {
        ...
        for (int i = 1, mask = 2; i < 8; i++, mask <<= 1)
          if (block[i].v != block[i-1].v)
            difference |= mask;
        Serial1.write(difference);

        for (int i = 0; i < 8; i++)
          if (1 & (difference >> i)) {
            Serial1.write(block[i].b);
            Serial1.write(block[i].g);
            Serial1.write(block[i].r);
          }
      }
      resume();
